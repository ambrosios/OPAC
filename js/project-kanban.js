...
class Kanban {
    ...
    async loadProject() {
        this.project = await window.app.projects.getById(this.projectId);
    ...
    async loadTasks() {
        try {
            let req = {};
            if(this.projectId) {
                req = await window.app.tasks.getByProjectId(this.projectId);
            } else {
                req = await window.app.tasks.getAll();
            }
            if(req.success) {
                this.tasks = req.data;
    ...
    async createTaskCard(task) {
        ...
        let projectHTML = '';
        if (!this.projectId) {
            const project = await window.app.projects.getById(task.project_id);
            if(project.success) {
    ...
    async handleDrop(e) {
        ...
        if (this.draggedTask.status !== newStatus) {
            ...
            // await window.app.tasks.updateTask(this.projectId, this.draggedTask.id, this.draggedTask);
            const req = await window.app.tasks.updateStatus(this.draggedTask.id, newStatus);
            if(req.success) {
                console.log("✅ Tâche", this.draggedTask.title, "modifié de", exStatus, "à", newStatus);
                const progressProject = await window.app.projects.updateProgressDate(this.draggedTask.project_id);
        ...
        try {
            if (this.editingTaskId) {
                ...
                if(existingTask.project_id !== taskData.project_id) {
                    const uProject = await window.app.tasks.updateProjectId(existingTask.id, updatedTask.project_id);
                    if(uProject.success) {
                ...
                if(existingTask.title !== taskData.title) {
                    const uTitle = await window.app.tasks.updateTitle(existingTask.id, updatedTask.title);
                    if(uTitle.success) {
                ...
                if(existingTask.description !== taskData.description) {
                    const uDesc = await window.app.tasks.updateDescription(existingTask.id, updatedTask.description);
                    if(uDesc.success) {
                ...
                if(existingTask.deadline !== taskData.deadline) {
                    const uDeadline = await window.app.tasks.updateDeadline(existingTask.id, updatedTask.deadline);
                ...
                if(existingTask.priority !== taskData.priority) {
                    const uPrio = await window.app.tasks.updatePriority(existingTask.id, updatedTask.priority);
                    if(uPrio.success) {
                ...
                if(existingTask.status !== taskData.status) {
                    const uStatus = await window.app.tasks.updateStatus(existingTask.id, updatedTask.status);
                    if(uStatus.success) {
                        console.log("✏️ ✅ Tâche", existingTask.id, ": Statut modifié");
                        const progressProject = await window.app.projects.updateProgressDate(existingTask.project_id);
                        if(progressProject.success) {
                            console.log("✅ Avancement du projet");
                        } else {
    ...
    async handleDeleteTask() {
        ...
        try {
            const req = await window.app.tasks.delete(this.editingTaskId);
...
async function load() {
    return await window.app.tasks.getAll();
}
console.log(load());