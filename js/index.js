...
async function loadStatDueDate() {
  ...
  const dueSoonTasks = await window.app.tasks.getDueSoon();
...
async function loadStatImportant() {
  ...
  const importantTasks = await window.app.tasks.getHighPriority();
...
async function handleSubmitTask(e) {
  ...
  try {
          // Mise à jour
          const existingTask = await window.app.tasks.getById(taskId);
          const updatedTask = {
              ...existingTask,
              ...taskData,
          ...
          if(existingTask.project_id !== taskData.project_id) {
              const uProject = await window.app.tasks.updateProjectId(updatedTask.id, updatedTask.project_id);
              if(uProject.success) {
          ...
          if(existingTask.title !== taskData.title) {
              const uTitle = await window.app.tasks.updateTitle(updatedTask.id, updatedTask.title);
              if(uTitle.success) {
          ...
          if(existingTask.description !== taskData.description) {
              const uDesc = await window.app.tasks.updateDescription(updatedTask.id, updatedTask.description);
              if(uDesc.success) {
          ...
          if(existingTask.deadline !== taskData.deadline) {
              const uDeadline = await window.app.tasks.updateDeadline(updatedTask.id, updatedTask.deadline);
          ...
          if(existingTask.priority !== taskData.priority) {
              const uPrio = await window.app.tasks.updatePriority(updatedTask.id, updatedTask.priority);
              if(uPrio.success) {
          ...
          if(existingTask.status !== taskData.status) {
              const uStatus = await window.app.tasks.updateStatus(updatedTask.id, updatedTask.status);
              if(uStatus.success) {
                  console.log("✏️ ✅ Tâche", updatedTask.id, ": Statut modifié");
                  const progressProject = await window.app.projects.updateProgressDate(updatedTask.project_id);
