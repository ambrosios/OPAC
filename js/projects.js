class Projects {
    ...
    async addNewProject(newProject) {        
        const result = await window.app.projects.create(newProject)
    ...
    async openProject(id) {
        try {
            document.location.href = "project.html?id=" + id;
        } catch (error) {
            showNotification('Erreur: ' + error.message, 'error');
        }
    }
...
class Project {
    ...
    async loadById(id) {
        try {
            const p = await window.app.projects.getById(id);
    ...
    async updateName(newName) {
        const req = await window.app.projects.updateName(this.id, newName);
    ...
    async updateDescription(newDescription) {
        const req = await window.app.projects.updateDescription(this.id, newDescription);
    ...
    async loadTasks() {
        const tasks = await window.app.tasks.getByProjectId(this.id);
        if(tasks.success) {
